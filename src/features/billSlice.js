  ...
  async (payload, thunkAPI) => {
    ...
    try {
      ...
      return res.data.sort(
        (a, b) =>
          new Date(a.date) - new Date(b.date) || a.name.localeCompare(b.name)
      );
    } catch (err) {
...
export const modifyBill = createAsyncThunk(
  "bill/modifyBill",
  async (payload, thunkAPI) => {
...
export const billPaidToggle = createAsyncThunk(
  "bill/billPaidToggle",
  async (payload, thunkAPI) => {
...
const billSlice = createSlice({
  ...
  reducers: {
    addBill: (state, { payload }) => {
      state.billItems = [...state.billItems, payload].sort(
        (a, b) =>
          new Date(a.date) - new Date(b.date) || a.name.localeCompare(b.name)
      );
    },
    ...
    setMonthlyBills: (state) => {
      ...
      state.billItems.forEach((bill) => {
        ...
        bill.dueDate.forEach((dueDate, index) => {
          ...
          if (
            new Date(
              dueDate.replace(/-/g, "/").replace(/T.+/, "")
            ).getMonth() === state.month &&
            new Date(dueDate).getFullYear() === state.year
          )
            monthlyBillsList.push({
              ...bill,
              dueDate: dueDate,
              paid: bill.paid[index],
              dueDateIndex: index,
      ...
      monthlyBillsList = monthlyBillsList.sort((a, b) =>
    ...
    setOpenBill: (state, { payload }) => {
      state.openBill = {
        ...payload,
        dueDate: new Date(payload.dueDate).toUTCString(),
      };
    },
  },
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addBill.fulfilled, (state, { payload }) => {
        state.isLoading = false;
        // console.log("Fulfilled: ", action)
        state.billItems = [...state.billItems, payload].sort((a, b) =>
          a.date > b.date ? 1 : -1
        );
      })
      ...
      .addCase(deleteBill.fulfilled, (state, { payload }) => {
        ...
        state.billItems = [...state.billItems, payload]
          .filter((account) => payload._id !== account._id)
      ...
      .addCase(modifyBill.fulfilled, (state, { payload }) => {
        state.isLoading = false;
        // console.log("Fulfilled: ", action)
        state.billItems = state.billItems
          .map((account) => (account._id === payload._id ? payload : account))
          .sort((a, b) => (a.date > b.date ? 1 : -1));
      })
